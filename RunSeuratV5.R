...
RunSeuratV5 <- function(
  ...
  obj.l <- mapply(
    ...
    FUN = function(counts.mat,
      ...
      # Add mouse data to meta.data
      mouse.data <- mouse.data[sample.name,,drop = FALSE]
      mouse.data <- as.data.frame(x = mouse.data[obj@meta.data$orig.ident,])
      rownames(x = mouse.data) <- NULL
      for (col.name in colnames(x = mouse.data)) {
        obj <- AddMetaData(
          object = obj,
          metadata = mouse.data[,col.name],
          col.name = col.name
        )
      }
  ...
  saveRDS(object = obj.merged,
          file = file.path(path,
                           project,
                           paste0(file.prefix,
                                  "_M3Drop_v3.10.6_danb_seurat_v5.1.0_sctransform_harmony_v1.2.0_unsupervised_clustering_PC1-",
