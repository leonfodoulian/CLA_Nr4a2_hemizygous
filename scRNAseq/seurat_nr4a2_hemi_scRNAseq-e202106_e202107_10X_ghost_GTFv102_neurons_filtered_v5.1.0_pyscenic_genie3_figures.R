...
regulon.params.l <- list(
  "enriched_regulons" = list(
    genes.to.highlight = c(
      "Car3", "Cdh13", "Col11a1", "Cux1", "Gfra1", "Gnb4", "Gng2", # downregulated
      "Lxn", "Nr2f2", "Nr4a2", "Pcdh8", "Satb1", "Synpr", # downregulated
      "Cnr1", "Cpeb1", "Fhl2", "Nfib", "Nos1ap", "Ryr2", "Syt17" # upregulated
    ),
    tf.to.keep = tf.levels[1:5], # top 5 CLA regulons enriched in WT cells
    panel.width = 10,
    panel.height = 3.75,
    ...
    if (save.plot) {
      FixSizeAndSave(plot = rss.contrast.plot,
                     filename = file.path("results/smim32_GTFv102/neurons_filtered/figures/temp",
                                          paste0(file.prefix, "_pyscenic_genie3_rss_contrast_", cell.type, ".pdf")),
                     is.ggassemble = FALSE,
                     panel.width = 6,
                     panel.height = 4.2,
                     unit.use = "cm",
...
FixSizeAndSave(plot = rss.contrast.plot,
               filename = file.path("results/smim32_GTFv102/neurons_filtered/figures/temp",
                                    paste0(file.prefix, "_pyscenic_genie3_rss_contrast.pdf")),
               is.ggassemble = FALSE,
               panel.width = 6,
               panel.height = 4.2,
               unit.use = "cm",
...
FixSizeAndSave(plot = rss.plot,
               filename = file.path("results/smim32_GTFv102/neurons_filtered/figures/temp",
                                    paste0(file.prefix, "_pyscenic_genie3_rss.pdf")),
               is.ggassemble = FALSE,
               panel.width = 6,
               panel.height = 4.2,
               unit.use = "cm",
...
tf.target.plot.l <- lapply(
  ...
  FUN = function(regulon.params,
    ...
    FixSizeAndSave(plot = patchwork::wrap_plots(tf.target.plot),
                   filename = file.path("results/smim32_GTFv102/neurons_filtered/figures/temp",
                                        paste0(file.prefix, "_pyscenic_genie3_tf_target_", regulon.params$file.suffix)),
                   is.ggassemble = TRUE,
...
FixSizeAndSave(plot = hemi.vln.plots,
               filename = file.path("results/smim32_GTFv102/neurons_filtered/figures/temp",
                                    paste0(file.prefix, "_pyscenic_genie3_violin_plots_hemi_modulated_tf.pdf")),
               is.ggassemble = TRUE,
               panel.width = 0.5,
